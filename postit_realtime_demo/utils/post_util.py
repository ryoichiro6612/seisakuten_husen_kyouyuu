...
class getPostits:
    ...
    def __init__(self, frame, outer_size):
        ...
        binImage = cv2.adaptiveThreshold(grayImage,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY,5,2.5)
        contours, hierarchy = cv2.findContours(binImage, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)

        bin_img = cv2.resize(binImage, (int(binImage.shape[1] * 0.25), int(binImage.shape[0]*0.25)))
        cv2.imshow("bin", bin_img)
        cv2.imwrite("bin.jpg", binImage)
        ...
        for count in range(0, len(contours)):
            ...
            #if  1500 > area > 300:
                #print area
                #cv2.polylines(frame, [box], True, (0, 255, 255), 5)
            ...
            if area > outer_size * outer_lower and area < outer_size * outer_upper:
               ...
               if idx != -1:
                   ...
                   if idx2  != -1:
                        ...
                        cv2.putText(frame, str(area), (box[0][0], box[0][1]), cv2.FONT_HERSHEY_PLAIN, 3.0, (0,128,0), 5)
...
class readDots():
    ...
    outer_size = 2100 * (expand_ratio ** 2)
    ...
    def read_bit(self, postit, first_x, first_y, horizon):
        ...
        if horizon == True:
            ...
            #cv2.imshow("id search", search_area)
            #cv2.waitKey(0)
            ...
            for count in range(0, len(contours)):
                ...
                #print self.outer_size
                #if 1:
                    #cv2.drawContours(search_area,[box],3,(255,255,0),5)
                    #calc left-top
