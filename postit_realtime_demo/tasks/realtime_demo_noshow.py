...
from utils import csv_util, post_util_noshow
...
def main():
    ...
    while (1):
        ...
        if time % fps == 0:
            ...
            # get postit
            getPostits = post_util_noshow.getPostits(frame, outer_size)
            ...
            for i in range(0, len(postit_points)):
                ...
                bit_array = post_util_noshow.readDots(postit_image_analyzing).bit_array
            ...
            for key in postit_saved:
                ...
                for i in range(0, len(postit_saved[key]["points"])):
                    relative_point = postit_saved[key]["points"][i] - desk_area[0][0]
                    relative_point[0] = int(cols * 1.0 * relative_point[0] / desk_width)
                    relative_point[1] = int(rows * 1.0 * relative_point[1]/ desk_height)
                ...
                np.mean(postit_relative_points, axis=0).astype(np.int)[0] - 40,
                np.mean(postit_relative_points, axis=0).astype(np.int)[1]), cv2.FONT_HERSHEY_PLAIN, 5.0,
                            (0, 240, 0), 5)
                #cv2.drawContours(projector_image, [np.array(postit_relative_points).astype(np.int)], 0,
                #                 (0, 0, 255), 2)
                #cv2.putText(projector_image, str(key), (
                #np.mean(postit_relative_points, axis=0).astype(np.int)[0] - 40,
                #np.mean(postit_relative_points, axis=0).astype(np.int)[1]), cv2.FONT_HERSHEY_PLAIN, 5.0,
